	...
	while test $n_slaves -le $n_slaves_max; do
		for pt in 0 1; do
			if test $pt -eq 0; then
				...
				echo "\${SOCBLOX}/units/interconnects/${protocol}/${protocol}_interconnect_${n_masters}x${n_slaves}.sv" >> ${protocol}_interconnects.f
			else
				perl scripts/mk_interconnect.pl -f \
					-o "${protocol}_interconnect_${n_masters}x${n_slaves}_pt.sv" \
					-n_masters $n_masters -n_slaves $n_slaves -default-slave-passthrough
				echo "\${SOCBLOX}/units/interconnects/${protocol}/${protocol}_interconnect_${n_masters}x${n_slaves}_pt.sv" >> ${protocol}_interconnects.f
			fi
		done
